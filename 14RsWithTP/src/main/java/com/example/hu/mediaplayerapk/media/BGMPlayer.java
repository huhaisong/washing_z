...
public class BGMPlayer implements MediaPlayer.OnPreparedListener, IMediaPlayer, MediaPlayer.OnCompletionListener, MediaPlayer.OnErrorListener {
    ...
    public void startPlay() {
        if (fileList.size() > 0) {
            play(fileList.get(0), false, false, false, 0);
        }
    }
    ...
    private void nextMusic() {
        ...
        if (fileList.size() > 0) {
            play(fileList.get(musicIndex), false, false, false, 0);
        }
    }

    @Override
    public void play(String url, boolean isFD, boolean isLoop, boolean enableSeek, int seekPosition) {
        Log.e("aaa", "---------------playBGM");
        if (mediaPlayer != null) {
            if (mediaPlayer.isPlaying())
                stop();
